...
import { MergedCrudOptions, CrudRequest } from "@nestjsx/crud/lib/interfaces";
    ...
    implements NestInterceptor
{
    intercept(context: ExecutionContext, next: CallHandler) {
        ...
        try {
            if (!req[PARSED_CRUD_REQUEST_KEY]) {
                const { ctrlOptions, crudOptions, action } =
                    this.getCrudInfo(context);
                ...
                if (!isNil(ctrlOptions)) {
                    ...
                    parser.search = {
                        $and: this.getSearch(parser, crudOptions, action),
                    };
                }
            ...
            if (!req[PARSED_CRUD_REQUEST_KEY]?.user) {
                req[PARSED_CRUD_REQUEST_KEY].user = {
    ...
    getParamsSearch(
        ...
        if (params) {
            parser.parseParams(params, (crudOptions as any).params);
    ...
    getAuth(
        ...
        if (crudOptions.auth) {
            ...
            if (isFunction(crudOptions.auth.or)) {
                auth.or = (crudOptions as any).auth.or(userOrRequest);
            }

            if (isFunction(crudOptions.auth.filter) && !auth.or) {
                auth.filter =
                    (crudOptions as any).auth.filter(userOrRequest) ||
                    /* istanbul ignore next */ {};
            }

            if (isFunction(crudOptions.auth.persist)) {
                parser.setAuthPersist(
                    (crudOptions as any).auth.persist(userOrRequest)
                );
            }
        }
